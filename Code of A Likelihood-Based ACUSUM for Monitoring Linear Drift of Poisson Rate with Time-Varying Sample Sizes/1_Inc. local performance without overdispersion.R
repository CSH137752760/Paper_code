...
for(i in 1:length(ctheta)){
  gamma=c_gamma[4]
  theta=ctheta[i]
  T14[i,1]=ACUSUM_VN1.ARL1.f(theta,cACUSUM_VN1.h[4],gamma,lambda0,ARL0,ii,N)
  T14[i,2]=ACUSUM_VN2.ARL1.f(theta,cACUSUM_VN2.h[4],gamma,lambda0,ARL0,ii,N)
  T14[i,3]=ACUSUM_VN3.ARL1.f(theta,cACUSUM_VN3.h[4],gamma,lambda0,ARL0,ii,N)
}
#----------TWS-------------------
for(i in 1:length(ctheta)){
  theta=ctheta[i]
...
rownames(Table2)=c("h","0.0000","0.0001","0.0002","0.0005","0.001","0.002","0.005","0.01","0.02","0.05","0.1","0.2","0.5")
Table2
write.csv(crh,file="T2_Inc_h.csv")
